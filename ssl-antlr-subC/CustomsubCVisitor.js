...
class CustomsubCVisitor extends subCVisitor {
    constructor() {
        super();
    ...
    visitMulDiv(ctx) {
        ...
        return ctx.op.type === subCParser.MUL ? left * right : left / right;
    }

    visitAddSub(ctx) {
        const left = this.visit(ctx.expr(0));
        const right = this.visit(ctx.expr(1));
        return ctx.op.type === subCParser.ADD ? left + right : left - right;
    }
}

export default CustomsubCVisitor;
